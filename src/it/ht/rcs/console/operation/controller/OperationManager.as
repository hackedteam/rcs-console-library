package it.ht.rcs.console.operation.controller
{
    ...
    public function unlistenPush():void
    {
      PushController.instance.removeEventListener( PushEvent.OPERATION, onOperationPush);
		...
		public static function get instance():OperationManager
		{
			return _instance;
		}
      ...
      var o:Operation;
		...
		public function closeOperation(id:String):void
		{
			...
			var targets:ListCollectionView=TargetManager.instance.getView(null, function(item:Object):Boolean
			{
				return (item is Target && item.status == 'open' && item.path[0] == id);
			});

			items=items.concat(targets.toArray());

			var agents:ListCollectionView=AgentManager.instance.getView(null, function(item:Object):Boolean
			{
				return (item is Agent && item.status == 'open' && item.path[0] == id);
			});
